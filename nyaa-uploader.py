...
def get_args():
    ...
    tosho_group.add_argument('--up-tosho', help='Submit a torrent already uploaded to Nyaa to tokyotosho.', action="store_true")
    parser.add_argument('cat', choices=["lraw", "lsub", "araw", "asub", "asnn"], help="Nyaa/Tosho Category")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-V', '--video', help="Video file torrent is named for.")
    group.add_argument('-l', '--local', help="Use video/torrent in calling directory. Must be exactly one.", action='store_true')
    parser.add_argument('-T', '--torrent', help="Torrent file, if it doesn't match video.")
    parser.add_argument('-C', '--creds', help="Credentials file.")
...
def get_settings():
    ...
    try:
        ...
        yaml_location = path.join(script_dir, args.creds + ".yaml")
...
if __name__ == "__main__":
    ...
    meta_payload = dict(redirect=view_url, alias="", submission_key="", tid=tid,
                        ...
                        namemod=video, infomod=url, descmod=nyaa_desc)
